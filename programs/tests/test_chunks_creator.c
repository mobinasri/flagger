...
bool testCreatingChunks(char* covPath){
	...
	ChunksCreator *chunksCreator = ChunksCreator_constructFromCov(covPath, NULL, chunkCanonicalLen, nThreads, windowLen);
	...
	for(int chunkIndex=0; chunkIndex < numberOfChunks; chunkIndex++){
		...
		if (strcmp(chunk->ctg, "ctg1") == 0){
			for(int windowIndex=0; windowIndex < chunk->coverageInfoSeqLen; windowIndex++){
				...
				fprintf(stderr,"**** %d %d %d %d\n",coverageInfo->coverage, coverageInfo->coverage_high_mapq, coverageInfo->coverage_high_clip, coverageInfo->annotation_flag);
...
bool test_ChunksCreator_parseHeaderInformation(const char* outputPath){
	...
	if(chunksCreator->numberOfRegions != 3) return false;
        bool correct = true;
        for(int i=0; i<2; i++){
                correct &= (strcmp((char *)stList_get(chunksCreator->annotationNames, i), (char *)stList_get(annotationNames,i)) == 0);
        }
	for(int i=0; i<3; i++){

...
bool test_BiasDetector_getAnnotationToRegionMap(char* bamPath, char *jsonPath){
	...
	int minMapq = 10;
	int minClip = 0.1;
	stHash *blockTable = ptBlock_multi_threaded_coverage_extraction_with_zero_coverage_and_annotation(bamPath, jsonPath, threads, minMapq, minClip);
	...
	BiasDetector *biasDetector = BiasDetector_construct(annotationNames, baselineAnnotationName, minCoverage, minTotalCount, minCovDiffNormalized);

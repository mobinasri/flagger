...
void splitCov(char *covPath, char *prefix, stHash *contigLenTable, int windowLen) {
    ...
    while (readNextBlock(fp, &contig, &contigLength, &start, &end, &cov) == 1) {
        ...
        if (blockLen < remainingWindowLen) {
            ...
            int remainingBlockLen = blockLen;
            int start_ = start;
            while (remainingWindowLen <= remainingBlockLen) {
                if (remainingWindowLen > 0) {
// save last block in the currect cov file and close the currect cov file
                    fprintf(fo,
                            "%d\t%d\t%d\n", start_, start_ + remainingWindowLen - 1, cov);
                }
                fflush(fo);
                fclose(fo);
// rename the saved cov file since we now know the end location
                sprintf(outputPath,
                        "%s.%s_%d_%d.cov", prefix, contig, fileStart, start_ + remainingWindowLen - 1);
                rename("tmp.cov", outputPath);
                printf("%s\t%d\t%d\n", contig, fileStart, start_ + remainingWindowLen - 1);
                fprintf(stderr,
                        "window finished, %s_%d_%d\n", contig, fileStart, start_ + remainingWindowLen - 1);
                ...
                sum +=
                        remainingWindowLen;
                start_ += remainingWindowLen;
                if (remainingContigLen > 0) {
                    fo = fopen("tmp.cov", "w");
                    fprintf(fo, ">%s %d\n", contig, contigLength);
                    fileStart = start_;
                    windowIdx += 1;
                    if (windowIdx < nWindows - 1) {
                        ...
                        remainingWindowLen = remainingContigLen;
                    }

                    fprintf(stderr,
                            "sum=%d,remainingContigLen = %d, remainingWindowLen= %d\n", sum, remainingContigLen,
                            remainingWindowLen);

                    if ((end - start_) + 1 < remainingWindowLen && start_ <= end) {
                        remainingWindowLen -= (end - start_) + 1;
                        remainingContigLen -= (end - start_) + 1;
                        remainingBlockLen -= (end - start_) + 1;
                        sum += (end - start_) + 1;
                        fprintf(fo, "%d\t%d\t%d\n", start_, end, cov);
    ...
    while (~(c = getopt(argc, argv, "c:p:f:s:h"))) {
        switch (c) {
            ...
            help:
                ...
                fprintf(stderr,
                        "         -f         a 2-column tab-delimited file (1st: contig name, 2st: effective contig length)\n");

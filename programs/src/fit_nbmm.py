...
"""
def plot_dist(cov_counts, model_fit, model_truth, figure_x_max, out_prefix, info):
    ...
    plt.savefig(f"{out_prefix}/{out_prefix}.fit_model.pdf")
    plt.show()  


def plot_comps(cov_counts, model_fit, contain_dup, simulate, figure_x_max, out_prefix, info):
    ...
    os.makedirs(f"{out_prefix}", exist_ok=True)
    plt.savefig(f"{out_prefix}/{out_prefix}.fit_model.components.pdf")
    plt.show()  
...
"""
def plot_log_likelihood(ll, init_iters_actual, acc_iters_actual, post_iters_actual, simulate, n_obs, out_prefix, info):
    color_init = "black"
    color_acc = "blue"
    color_post = "red"
    ...
    os.makedirs(f"{out_prefix}", exist_ok=True)
    plt.savefig(f"{out_prefix}/{out_prefix}.loglikelihood.pdf")
    plt.show()


def write_probabilities(cov_counts, model_fit, simulate, contain_dup, out_prefix):
    print(f"Writing output files (output dir = {out_prefix}) ...")
    ...
    if simulate:
        with open(f"{out_prefix}/{out_prefix}.counts","w+") as f:
    ...
    if contain_dup:
        with open(f"{out_prefix}/{out_prefix}.coverage_component_probs.tsv","w+") as f:
...
"""
def write_loglikelihood(ll, aic, bic, init_iters_actual, acc_iters_actual, post_iters_actual, out_prefix):
    ...
    os.makedirs(f"{out_prefix}", exist_ok=True)
    with open(f"{out_prefix}/{out_prefix}.loglikelihood.tsv","w+") as f:
...
def main():
    ...
    parser.add_argument('--out-prefix', type=str, default = "nbmm_test",
    ...
    if not args.disable_plot:
        ...
        plot_dist(cov_counts, model_fit, model_truth, figure_x_max, out_prefix, info)

        plot_comps(cov_counts, model_fit, contain_dup, simulate, figure_x_max, out_prefix, info)
    
        # plot loglikelihood w.r.t effective iterations
        plot_log_likelihood(ll, init_iters_actual, acc_iters_actual, post_iters_actual, simulate, n_obs, out_prefix, info)

    write_probabilities(cov_counts, model_fit, simulate, contain_dup, out_prefix)    
    write_loglikelihood(ll, aic, bic, init_iters_actual, acc_iters_actual, post_iters_actual, out_prefix)

...
class HomologyBlock:
    ...
    def updateAnnotationBlocksForSampling(self):
        ...
        wholeBlockWithoutMargins = BlockList([(1, self.origEnd - self.origStart + 1)]).truncateFromLeft(
            wholeBlockMarginFromLeft, inplace=False)
        ...
        wholeBlockWithoutMarginsForMisjoin = BlockList([(1, self.origEnd - self.origStart + 1)]).truncateFromLeft(
            wholeBlockMarginFromLeft, inplace=False)
        ...
        for name in self.annotationBlockLists:
            ...
            self.annotationStartBlockListsForSampling[name].removeBlocksShorterThan(lengthToTruncateFromRight,
                                                                                    inplace=True)
            ...
            self.annotationStartBlockLengthsForSampling[name] = [i[1] - i[0] + 1 for i in
                                                                 self.annotationStartBlockListsForSampling[name].blocks]
            ...
            # truncate the blocks to make sure they do not have overlap with the margins
            self.annotationBlockListsForSamplingMisjoin[name].intersect(wholeBlockWithoutMarginsForMisjoin,
                                                                        inplace=True)
            self.annotationBlockLengthsForSamplingMisjoin[name] = [i[1] - i[0] + 1 for i in
                                                                   self.annotationBlockListsForSamplingMisjoin[
                                                                       name].blocks]
            self.annotationTotalLengthsForSamplingMisjoin[name] = sum(
                self.annotationBlockLengthsForSamplingMisjoin[name])
...
class HomologyRelation:
    ...
    def __init__(self, block: HomologyBlock, homologousBlock: HomologyBlock, cigarList: list, orientation: str):
        ...
        if cigarList != None and len(cigarList) > 0:
            ...
            alignment = Alignment(
                f"query_ctg\t{homologousBlockLen}\t0\t{homologousBlockLen}\t{orientation}\tref_ctg\t{blockLen}\t0\t{blockLen}\t0\t0\t60\tcg:Z:{makeCigarString(cigarList)}\ttp:A:P")
...
class HomologyRelationChains:
    ...
    @staticmethod
    def createAllInclusiveRelationChainsFromAlignments(alignments: list, contigLengths: dict,
                                                       newCtgSuffix: str) -> defaultdict:
        """
        ...
        for alignment in alignments:
            ...
            relationFreeIntervals[ref2queryRelation.block.newCtg].subtract(
                BlockList([(alignment.chromStart + 1, alignment.chromEnd)]), inplace=True)
            relationFreeIntervals[query2refRelation.block.newCtg].subtract(
                BlockList([(alignment.contigStart + 1, alignment.contigEnd)]), inplace=True)
        ...
        for origCtg in contigLengths:
            for interval in relationFreeIntervals[origCtg + newCtgSuffix].blocks:
                voidRelation = HomologyRelation.createVoidRelationFromInterval(origCtg, interval[0], interval[1],
                                                                               newCtgSuffix)
    ...
    def fillAnnotationBlockListsFromOriginalContigs(self, annotationBlockListsPerOrigContig, origContigLengths,
                                                    newCtgSuffix):
        """
...
def induceMisjoinMisAssembly(self, newCtg_1, orderIndex_1, loc_1, newCtg_2, orderIndex_2, loc_2,
...
def induceDuplicationMisAssembly(self, newCtg, orderIndex, duplicationStart, duplicationEnd):
    ...
    rBlockPart2Dup.addMisAssemblyBlockList(
        BlockList([(1, rBlockPart2Dup.origEnd - rBlockPart2Dup.origStart + 1, "Dup")]))
...
def updateAnnotationBlocksForSampling(self, annotations, misAssemblyLength, minOverlapRatioWithEachAnnotation,
                                      minMarginLength):
    """
    ...
    for annotation in annotations:
        if minBlockSizes is None:
            ...
            for minBlockSize in minBlockSizes:
                totalCounts[annotation].append(
                    self.getTotalCountOfLongerBlocks(annotation, minBlockSize, onlyRefInHomology))
    return totalCounts
...
def getTotalLengthOfLongerBlocksForAllAnnotations(self, annotations, minBlockSizes, onlyRefInHomology=False):
    ...
    for annotation in annotations:
        if minBlockSizes is None:
            ...
            for minBlockSize in minBlockSizes:
                totalLengths[annotation].append(
                    self.getTotalLengthOfLongerBlocks(annotation, minBlockSize, onlyRefInHomology))
    return totalLengths

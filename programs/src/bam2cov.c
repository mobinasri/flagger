...
int main(int argc, char *argv[]) {
    ...
    while (~(c = getopt_long(argc, argv, "i:t:j:m:r:f:o:g:c:b:d:g:a:uh", long_options, NULL))) {
		...
		break;
            ...
            help:
                ...
                fprintf(stderr, "         -j, --annotationJson                 Json file for the annotation bed files. At least one BED file should be in this json and it can be a BED file covering the whole genome/assembly [maximum 58 bed files can be given and the keys can be any number between 1-58 for example {\"1\":\"/path/to/1.bed\", \"2\":\"/path/to/2.bed\"}]\n");
                fprintf(stderr, "         -m, --mapqThreshold                  Minimum mapq for the measuring the coverage of the alignments with high mapq [Default = 20]\n");
                fprintf(stderr, "         -c, --clipRatioThreshold             Minimum clipping ratio for the measuring the coverage of the highly clipped alignments [Default = 0.1]\n");
                fprintf(stderr, "         -f, --format                         If this parameter is enabled and output file extension is either cov or cov.gz then the output will be formatted based on the value of this parameter. options: [\"all\", \"only_total\", \"only_high_mapq\"][Default = \"all\"]\n");
		fprintf(stderr, "         -t, --threads                        Number of threads [default: 4]\n");
                fprintf(stderr, "         -o, --output                         Output path [output file can be either cov/cov.gz/bed/bed.gz]\n");
		fprintf(stderr, "         -r, --restrictBiasAnnotations        Path to a text file that contains one annotation name per line. Only these annotations will be considered for detecting coverage biases. If no file is provided it will run bias detection on all annotations in the json file.\n");
		fprintf(stderr, "         -b, --baselineAnnotation             [coverage bias detection] name of the baseline annotation. If no baseline name is provided it will consider all blocks with no given annotation as baseline.\n");
                fprintf(stderr, "         -d, --covDiffThreshold               Threshold for reporting an annotation as biased or not  (It is being applied on the coverage deviation normalized by the baseline coverage) [default:0.15]\n");
                fprintf(stderr, "         -g, --minBiasCoverage                [coverage bias detection] the most frequent coverage will selected from among the coverages greater than or equal to the value of this parameter (It can be useful for ignoring the regions with no coverage) [default:5]\n");
		fprintf(stderr, "         -l, --minBiasLength                  [coverage bias detection] the minimum length of an annotation to be considered biased [default:100000 (100k)]\n");
		fprintf(stderr, "         -u, --runBiasDetection               Run coverage bias detection. It will update the number of regions. [Default: disabled]\n");
                return 1;
        }
    }
    ...
    fprintf(stderr, "[%s] Number of annotations added to the blocks table (+1 for 'no_annotation' which is a reserved annotation name for blocks with no annotation): %d\n", get_timestamp(), stList_length(annotationNames));

    char *extension = extractFileExtension(outPath);

    if (runBiasDetection && jsonPath == NULL){
	    fprintf(stderr, "[%s] (Warning) Json file is not provided so for bias detection there is only one annotation ('no_annotation')!\n", get_timestamp());
    }
    ...
    if (runBiasDetection){ 
		    ...
		    fprintf(stderr, "[%s] --restrictBiasAnnotations is not provided so all annotations will be checked for coverage bias!\n", get_timestamp());
		...
		BiasDetector *biasDetector = BiasDetector_construct(annotationNames, baselineAnnotationName, minBiasCoverage, minBiasLength, covDiffNormalizedThreshold);
	    ...
	    fprintf(stderr, "[%s] Bias detection is disabled. Running bias module only for getting the whole genome median coverage.\n", get_timestamp());

            BiasDetector *biasDetector = BiasDetector_construct(annotationNames, annotationZeroName, 1, 1, covDiffNormalizedThreshold);
            BiasDetector_setStatisticsPerAnnotation(biasDetector, blockTable);
    ...
    stList *headerLines = ptBlock_create_headers(annotationNames, coveragePerRegion, numberOfRegions, numberOfLabels, isTruthAvailable);

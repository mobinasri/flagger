...
void add_coverage_info_to_all_annotation_block_tables(stList *block_table_list){
    for(int i=0; i < stList_length(block_table_list); i++){
        ...
        ptBlock_add_data_to_all_blocks_stHash(blocks_per_contig,
                                              cov_info,
...
int main(int argc, char *argv[]) {
    ...
    add_coverage_info_to_all_annotation_block_tables(stList *annotation_block_table_list);
    ...
    for(int i=0; i < stList_length(block_table_list); i++){
        ptBlock_extend_block_tables(whole_genome_block_table, stList_get(block_table_list, i));
    }

    fprintf(stderr, "[%s] Started sorting and merging blocks after adding annotation and whole-genome 0-coverage blocks.\n", get_timestamp());
    //sort
    ...
    fprintf(stderr, "[%s] Final block table is created.\n", get_timestamp());

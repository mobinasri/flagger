...
BiasDetector *BiasDetector_construct(stList *annotationNames,
	...
	biasDetector->countDataPerAnnotation = CountData_construct1DArray(MAX_COVERAGE_VALUE, biasDetector->numberOfAnnotations);
...
void BiasDetector_setCountDataPerAnnotation(BiasDetector *biasDetector, stHash *blockTable){
    ...
    while ((contigName = stHash_getNext(it)) != NULL) {
	    ...
	    for(int annotationIndex=0; annotationIndex < biasDetector->numberOfAnnotations; annotationIndex++){
                if (CoverageInfo_overlapAnnotationIndex(covInfo, annotationIndex)){ // checking overlap is fast (a few bit-wise operations)
...
void BiasDetector_setMostFrequentCoveragePerAnnotation(BiasDetector *biasDetector){
    for (int annotationIndex = 0; annotationIndex < biasDetector->numberOfAnnotations; annotationIndex++) {
        int mostFreqCoverage = CountData_getMostFrequentValue(biasDetector->countDataPerAnnotation[annotationIndex], biasDetector->minCoverage, MAX_COVERAGE_VALUE);
...
void BiasDetector_setMaxCountPerAnnotation(BiasDetector *biasDetector){
    for (int annotationIndex = 0; annotationIndex < biasDetector->numberOfAnnotations; annotationIndex++) {
        int maxCount = (int) CountData_getMaxCount(biasDetector->countDataPerAnnotation[annotationIndex], biasDetector->minCoverage, MAX_COVERAGE_VALUE);
...
void BiasDetector_setTotalCountPerAnnotation(BiasDetector *biasDetector){
    for (int annotationIndex = 0; annotationIndex < biasDetector->numberOfAnnotations; annotationIndex++) {
        int totalCount = (int) CountData_getTotalCount(biasDetector->countDataPerAnnotation[annotationIndex], 0, MAX_COVERAGE_VALUE);
...
void BiasDetector_runBiasDetection(BiasDetector *biasDetector,
    ...
    if (biasDetector->statsAreUpdated == false){
	    fprintf(stderr, "[%s] (Warning) Bias detection cannot be executed. Run the function BiasDetector_setStatisticsPerAnnotation() for updating stats.\n", get_timestamp());
	    return;
    }
    ...
    if(annotationNamesToCheck == NULL || stList_length(annotationNamesToCheck) == 0){
            fprintf(stderr, "[%s] (Warning) annotationNamesToCheck is empty for bias detection. All annotations are mapped to region index 0!\n",get_timestamp());
    ...
    if(tsvPathToWriteTable == NULL){
            fprintf(stderr, "[%s] (Warning) Summary table for coverage bias detection will not be written to a file since no path is provided!\n",get_timestamp());
    }else{
    ...
    if(tsvFile != NULL){
    	fprintf(stderr, "[%s] Printing coverage bias table for the given annotations into this file %s \n", get_timestamp(), tsvPathToWriteTable);
    	fprintf(tsvFile, "annotation\tstatus\tmost_freq_cov\tmax_count\ttotal_count\tcov_diff_normalized\tregion_index\n");
    }
	...
	bool isBiased = (biasDetector->minCovDiffNormalized < covDiffNormalizedAbs) && (biasDetector->minTotalCount < totalCount) && mostFrequentCoverage != -1;

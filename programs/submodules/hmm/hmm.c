...
HMM* HMM_construct(int nClasses, int nComps, int nEmit, int* nMixtures, VectorDouble**** mu, VectorDouble*** muFactors, MatrixDouble*** covFactors, double maxHighMapqRatio, MatrixDouble** transNum, MatrixDouble** transDenom){
	...
	for(int r=0; r < nClasses; r++){
		for(int c1=0; c1 < nComps; c1++){
			for(int c2=0; c2 < nComps; c2++){
                		model->trans[r]->data[c1][c2] = (1 - model->terminateProb) * model->transNum[r]->data[c1][c2] / model->transDenom[r]->data[c1][c2];
			}
		...
		for(int c2=0; c2 < nComps; c2++){
			for(int c1=0; c1 < nComps; c1++){ // Transition from c1 comp to c2 comp
				if (seqClass[i-1] != seqClass[i]) { // if the class has changed
					...
					if (model->maxHighMapqRatio < highMapqRatio){ // Due to high number of high mapq it cannot be false duplication
						// probs are getting renormalized exlcuding transision to false duplication (state index = 1)
                                                tProb = c2 == 1 ? 0.0 : trans[seqClass[i]]->data[c1][c2] / (1 - trans[seqClass[i]]->data[c1][1]);
                                        }
                                        else{// ratio is low so it can be false duplication so all probs are being used
			...
			for(int m = 0; m < gaussian->n; m++){
				...
				for(int j=0; j < nEmit; j++){
					if (muFactor->data[j] < 0) continue;
						...
						gaussian->mu[m]->data[j] = muPosFactor->data[j] / muPosFactorWeight->data[j] * muFactor->data[j];
					}
				...
				for(int j1 = 0; j1 < nEmit; j1++){
						...
						if (factor < 0) continue;
                                                if (0 < factor && 1e2 < covPosFactorWeight->data[j1][j2]){
                                                        gaussian->cov[m]->data[j1][j2] = covPosFactor->data[j1][j2] / covPosFactorWeight->data[j1][j2] * factor; 
                                                }
						else if(1e2 < gaussian->covDenom[m]->data[j1][j2]){
                                                        gaussian->cov[m]->data[j1][j2] = gaussian->covNum[m]->data[j1][j2] / gaussian->covDenom[m]->data[j1][j2];
                                                }
                                        }
                                }
			}
...
int Chunk_addWindow(Chunk* chunk){
	...
	for(int j=0; j < windowSum->dim; j++){
        	chunk->seqEmit[chunk->seqLen]->data[j] = 255 < round(windowSum->data[j]) ? 255 : round(windowSum->data[j]); // windowSum is actually average here
	}
